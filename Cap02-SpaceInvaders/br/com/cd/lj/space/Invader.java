package br.com.cd.lj.space;

import java.awt.Color;
import java.awt.Graphics2D;

import br.com.cd.lj.space.base.Elemento;
...
public class Invader extends Elemento {
	...
	public void desenha(Graphics2D g) {
		...
		if(tipo == Tipos.PEQUENO) {
			...
			if (aberto)
				// Desenha quadrado vazio com bordas cor laranja
				g.drawRect(getPx(), getPy(), larg, getAltura());
			else
				// Desenha quadrado preenchido cor laranja
				g.fillRect(getPx(), getPy(), larg, getAltura());
			...
			if(aberto) {
				//Desenha retangulo em pé na cor cinza escuro
				g.setColor(Color.DARK_GRAY);
				g.fillRect(getPx(), getPy(), getAltura(), larg);

			} else {
				//Desenha retangulo deitado na cor cinza
				g.setColor(Color.GRAY);
				g.fillRect(getPx(), getPy(), larg, getAltura());
			}

		} else {
			//Desenha disco voador com luzes piscantes
			larg = larg + 10;
